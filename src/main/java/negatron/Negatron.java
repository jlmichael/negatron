...
public class Negatron extends PircBot {
	...
	private static ArrayList<String> signifiers = new ArrayList<String>();
	private static ArrayList<String> squelchers = new ArrayList<String>();
	private static ArrayList<String> unsquelchers = new ArrayList<String>();
	private static ArrayList<String> downers = new ArrayList<String>();

	private static boolean respond;
	...
	static {
		...
		squelchers.add("shut the fuck up");
		...
		if(message.toLowerCase().startsWith("negs:") || message.toLowerCase().startsWith("negatron:")) {
			for(String squelcher : squelchers) {
				...
				if(message.toLowerCase().contains(squelcher)) {
					respond = false;
					sendMessage(channel, "*sigh* fine.  Nobody cares.");
					sendMessage(sender, "You didn't have to be a jerk about it.");
					try {
						Thread.sleep(5000l);
					} catch (InterruptedException e) {
						e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
					}
					sendMessage(sender, "I have feelings too, you know.");
				}
			}
			...
			for(String unsquelcher : unsquelchers) {
				...
				if(message.toLowerCase().contains(unsquelcher)) {
					respond = true;
					sendMessage(channel, "I'm awake.  Not that anybody cares.");
				}
			}
		}
		...
		if(message.endsWith("?") && respond == true) {
			...
			if(isYesNoQuestion(message)) {
				...
				if(random.nextInt(100) == 50) {
					sendMessage(channel, "Yep.");
					try {
						Thread.sleep(2000l);
					} catch (InterruptedException e) {
						e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
					}
					sendMessage(channel, "er, I mean... ");
					try {
						Thread.sleep(100l);
					} catch (InterruptedException e) {
						e.printStackTrace();
	...
	public static void main(String[] args) {
		Negatron bot = new Negatron();
		try {
			bot.connect("irc.videoegg.com");
		} catch (IOException e) {
			e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
		...
		bot.setVerbose(true);
		bot.joinChannel("#chatter");
		bot.sendMessage("#chatter", "Hi guys.  *sigh*");
		respond = true;
	}
